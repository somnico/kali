...
zstyle ':fzf-tab:complete:cd:' popup-pad 100 100
# zstyle ':fzf-tab:complete:cat:' 
# zstyle ':completion:*:*:cat:*:argument-1' file-patterns "*(-.):files"
...
zstyle ':fzf-tab:*' fzf-bindings 'tab:toggle+down' 'shift-tab:toggle+up' 'space:accept'
...
export FZF_DEFAULT_OPTS="
  ...
  --pointer='▌' --marker='▌' --border-label='' --separator='' --scrollbar='' --info=default --multi
  --bind 'ctrl-l:clear-query,home:first,end:last,enter:accept-non-empty,left:close'
  --tmux center
"
...
# plocate "" | fzf --preview 'batcat --style=numbers --color=always {} || cat {}' --preview-window=right
...
ff() {
  ...
  fzf --height ~100% --tmux 70% --layout=reverse --multi --info=default --border \
...
fzf-history-widget() {
  ...
  selected_cmd=$(fc -rln 1 | fzf --height=70% --tmux 70% --exact --no-multi --no-sort --query "$LBUFFER" \
...
fzf-cd-widget() {
  ...
  temp_dir=$(
    ...
    fzf --height=80% --tmux 70% --layout=reverse --exact --no-multi \
